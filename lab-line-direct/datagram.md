...
```
ip addr show dev EXPIFACE1
```

on each host, and make a note of the MTU of the `EXPIFACE1` interface on each. 
...
```
sudo tcpdump -env -i EXPIFACE1 "not tcp"
```
...
```
sudo tcpdump -i EXPIFACE1 -w $(hostname -s)-no-ip-fragment.pcap "not tcp"
```
...
```
sudo tcpdump -i EXPIFACE1 -w $(hostname -s)-ip-fragment.pcap "not tcp"
```
...
```
sudo tcpdump -en -i EXPIFACE1 "not tcp"
```
